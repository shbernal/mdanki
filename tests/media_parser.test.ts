...
describe('MediaParser', () => {
  ...
  it('fails fast when remote media fetching is disabled', async () => {
    const parser = new MediaParser(path.join(tmpDir, 'card.md'), { allowRemoteMedia: false });

    await expect(parser.parse('<img src="http://example.com/test.jpg">')).rejects.toThrow(
      /remote media fetching is disabled/i,
    );
  });
  ...
  it('times out remote downloads', async () => {
    ...
    const parser = new MediaParser(path.join(tmpDir, 'card.md'), { remoteFetchTimeoutMs: 5 });

    await expect(parser.parse('<img src="http://slow.example.com/test.jpg">')).rejects.toThrow(
      /timed out/i,
    );
  ...
  it('wraps local file errors with path context', async () => {
    ...
    await expect(parser.parse(`<img src="${missing}">`)).rejects.toThrow(missing);
  });
});
