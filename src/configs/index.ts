...
const readTemplateSettings = (templatePath?: string): Partial<Settings> => {
  ...
  if (!fs.existsSync(resolved) || !fs.statSync(resolved).isDirectory()) {
    console.warn(
      `Template directory not found at ${resolved}, falling back to defaults.`,
    );
    return {};
  }
  ...
  for (const [key, fileName] of Object.entries(TEMPLATE_FILES) as [
    ...
    if (!fs.existsSync(filePath)) {
      console.warn(
        `Missing template file "${fileName}" in ${resolved}, falling back to defaults.`,
      );
      return {};
    }
    ...
    try {
      ...
      console.warn(
        `Failed to read template file "${fileName}" in ${resolved}:`,
        error,
      );
      return {};
    }
