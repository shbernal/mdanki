...
const cli = yargs(hideBin(process.argv))
  ...
  .command(
    ...
    (command) =>
      command
        ...
        .positional("target", {
          describe:
            "Path to the resulting .apkg file. When omitted, the file stem is used with .apkg in the current directory.",
          type: "string",
        }),
    async (argv) => {
      const deckArg = typeof argv.deck === "string" ? argv.deck : undefined;
      const templateArg =
        typeof argv.template === "string" ? argv.template : undefined;
  ...
  .example(
    "$0 study.md anki.apkg --deck Study",
    "Convert a single markdown file",
  )
  .option("template", {
    type: "string",
    description:
      "Template directory containing front.html, back.html and style.css",
  })
  .option("deck", {
    type: "string",
  ...
  .option("remote-media", {
    ...
    default: true,
    description:
      "Download media from remote http/https sources referenced in markdown",
  })
